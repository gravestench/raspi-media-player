...
async function loadAccount() {
  ...
  try {
    ...
    const genres = dashboardCard('Your genres'); const genreCloud = document.createElement('div'); genreCloud.className = 'genre-cloud'; (value.genres || []).forEach(item => { const tag = document.createElement('button'); tag.type = 'button'; tag.textContent = `${item.name} · ${item.count}`; tag.addEventListener('click', () => discover(item.name)); genreCloud.append(tag); }); if (!value.genres?.length) genreCloud.textContent = 'Play some enriched tracks to build your taste profile.'; genres.append(genreCloud);
